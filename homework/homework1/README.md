...
In addition to the final version, we considered and tested other approaches.
...
Rather than focusing on a single fixed threshold, the testing results show that the optimized implementation delivered a significant speedup compared to the basic version on the reference system used. The optimized version:

- ran in `76 ms` with default compilation,
- ran in `3 ms` with `-O3`,
- ran in `2 ms` with `-O3 -march=native -flto -funroll-loops`.

Compared to the baseline results of `112 ms`, `6 ms`, and `4 ms`, this represents a clear and consistent performance improvement across all tested compiler settings.
